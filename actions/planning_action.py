...
class CreateExecutionPlan(Action):
    ...
    async def run(self, instruction: str) -> list:
        ...
        Bạn đang quản lý đội ngũ gồm 5 Sub-agents chuyên biệt:
        ...
        4. "InvestmentAdvisor": Chuyên gia tư vấn đầu tư. Tổng hợp tất cả các phân tích để đưa ra khuyến nghị cuối cùng (MUA/BÁN/GIỮ) dưới dạng báo cáo nháp.
        5. "QualityAuditor": Tổ trọng tài kiểm toán. Đọc báo cáo nháp, đối chiếu với bối cảnh gốc để chống ảo giác (hallucination) và tối ưu độ dài.
        ...
        - LUÔN LUÔN để "InvestmentAdvisor" đóng vai trò viết báo cáo nháp dựa trên bối cảnh.
        - LUÔN LUÔN để "QualityAuditor" là bước cuối cùng phân tích chất lượng của nháp và xuất bản báo cáo thực tế.
        ...
        - "agent": Tên Agent (chỉ chọn: "SentimentAgent", "QuantAgent", "FundamentalAnalyst", "InvestmentAdvisor", "QualityAuditor").
        ...
        [
            ...
            {{"agent": "InvestmentAdvisor", "task": "Tổng hợp tất cả các phân tích và viết báo cáo nháp tư vấn đầu tư.", "ticker": "FPT"}},
            {{"agent": "QualityAuditor", "task": "Kiểm tra chéo số liệu và tối ưu hóa độ dài trước khi xuất bản bản chót.", "ticker": "FPT"}}
        ]
        """
        ...
        try:
            ...
            # Ưu tiên lấy cục json bên trong khối code json nếu có
            ...
            # Đảm bảo cắt các text phụ dư thừa từ LLM trước hoặc sau mảng.
